...
namespace DatabaseConnectionExample
{
    class Program
    {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        for (int i = 0; i < reader.FieldCount; i++)
                        {
                            ...
                            InsertData(kvp.Key, kvp.Value[k], reader[i]);
                            k++;
                        }
        ...
        public static void InsertData(string tableName, string columns, object r1)
        {
            string insertQuery = $"INSERT INTO {tableName} ({columns}) VALUES (@Value)";

            using (var cmd = new NpgsqlCommand(insertQuery, targetConnection))
            {

                cmd.Parameters.AddWithValue("@Value", r1);
