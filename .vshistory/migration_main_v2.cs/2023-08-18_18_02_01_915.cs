...
namespace DatabaseConnectionExample
{
    class Program
    {
            ...
            foreach (var kvp in columnNames)
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            InsertData(kvp.Key, kvp.Value, reader[i]);          ///////////////////
                            rowValues += $"{reader[i]}          ";
        ...
        public static void InsertData(string tableName, List<string> columns, object reader)
        {
            string insertQuery = $"INSERT INTO {tableName} ({columns[0]}) VALUES ({reader}) ";
