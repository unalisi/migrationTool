...
namespace DatabaseConnectionExample
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (kvp.Key != "tbl_stok_depo_miktar" && kvp.Key != "tbl_017_muh_banka" &&
                    kvp.Key != "tbl_015_muh_cari" && kvp.Key != "tbl_018_muh_proje" && kvp.Key != "tbl_parametreler" &&
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                string insertQuery =
                                    $"INSERT INTO {kvp.Key} ({selectColumns}) VALUES ({string.Join(", ", kvp.Value.Select(v => $"@{v}"))})";
