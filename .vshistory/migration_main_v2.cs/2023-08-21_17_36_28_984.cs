...
namespace DatabaseConnectionExample
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (string tableName in tableNames)
            {
                try
                {
                    ...
                    query =
                        ...
                        $"AND TABLE_NAME NOT LIKE ( 'LOG_%' OR '%_023%' OR '%_022%' OR '%_021%' OR '%_020%' " +
                        $"OR '%_019%' OR '%_018%' OR '%_017%'  OR '%_016%'  OR '%_015%' OR '%_014%' OR '%_013%' OR '%_012%' OR '%_011%' OR '%_010%')"/*AND TABLE_NAME NOT LIKE '%_023%' AND TABLE_NAME NOT LIKE '%_022%'*/;
                    using (SqlCommand command = new SqlCommand(query, connection))
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
            ...
            {
                if (kvp.Key != "tbl_stok_depo_miktar" && kvp.Key != "tbl_017_muh_banka" && kvp.Key != "tbl_015_muh_cari" && kvp.Key != "tbl_018_muh_proje"
                         ...
                         && kvp.Key != "tbl_015_muh_banka"
                         && kvp.Key != "tbl_015_muh_banka"
                         && kvp.Key != "tbl_015_muh_banka") // && kvp.Key != "tbl_015_muh_banka"
                {
