                ...
                if (kvp.Key == "tbl_rakip_firma")
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            {
                                //InsertData(kvp.Key, kvp.Value, reader[i], k);          ///////////////////
                                rowValues += $"{reader[i]}          ";
                                //sw.WriteLine(rowValues);
                                //Console.WriteLine(rowValues);


                                string checkTableQuery = "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = '" + kvp.Value + "')";

                                using (NpgsqlCommand cmd2 = new NpgsqlCommand(checkTableQuery, targetConnection))
                                {
                                    if ((bool)cmd2.ExecuteScalar()) // if table exists
                                    {
                                        string insertQuery = $"INSERT INTO {kvp.Key} ({kvp.Value[k]}) VALUES (@Value) ";
                                        using (var cmd = new NpgsqlCommand(insertQuery, targetConnection))
                                        {
                                            cmd.Parameters.AddWithValue("@Value", reader[i]);
                                            cmd.ExecuteNonQuery();
                                        }
                                        k++;
                                    }
                                    else                        // Tablo postgre yoksa döngüden çık
                                        break;
                                }
